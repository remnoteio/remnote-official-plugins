...
import { normalizeContent, timeSince } from "../lib/utils";
...
function RightSidebar2() {
	...
	return (
		...
		>
			...
			{remData
				.slice(0, NUM_TO_LOAD_IN_BATCH * numLoaded)
				.map((data, i) => (
					<RemHistoryItem
						data={data}
						remId={data.remId}
			...
			{numUnloaded > 0 && (
				...
				>
					{" "}
					Load more{" "}
					<span className="rn-clr-content-secondary">
						({numUnloaded})
					</span>
				</div>
			)}
...
function RemHistoryItem({
	...
	const [remContent, setRemContent] = React.useState<string>("");

	React.useEffect(() => {
		const fetchRemContent = async () => {
			const rem = await plugin.rem.findOne(remId);
			if (rem) {
				const content = rem.text;
				const normalizedContent = normalizeContent(content);
				setRemContent(normalizedContent);
			}
		};
		fetchRemContent();
	}, [remId, plugin]);
